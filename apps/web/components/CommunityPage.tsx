  ...
  useEffect(() => {
    ...
    async function loadCommunity() {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/learn/community${suffix}`, { headers: getStudentRequestHeaders(), cache: "no-store" });
        if (ignore) return;
        if (!response.ok) {
          setStatus("社区内容读取失败，请确认 FastAPI 服务正在运行。");
          return;
        }
        setHome((await response.json()) as CommunityHome);
        setStatus(query ? `正在查看包含“${query}”的内容` : "问题、解答、笔记和同学都在这里。");
      } catch {
      ...
      <div className="min-h-screen bg-[#f6fbf9]">
        ...
        <main className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
          <section className="panel rounded-lg p-8 text-center">
            ...
            <h1 className="mt-4 text-4xl font-black text-ink">学习社区</h1>
            <p className="mx-auto mt-3 max-w-2xl text-base leading-7 text-slate-600">登录学生账号后，可以查找问题、解答、共享笔记和同学。</p>
            <div className="mt-6 flex justify-center gap-3">
              <Link href="/login" className="focus-ring rounded-lg bg-coral px-5 py-3 text-sm font-black text-white">学生登录</Link>
              <Link href="/register" className="focus-ring rounded-lg border border-slate-200 bg-white px-5 py-3 text-sm font-black text-ink">注册账号</Link>
            </div>
          </section>
        </main>
      </div>
          ...
          <div className="grid gap-4 bg-[linear-gradient(135deg,#fff8ef_0%,#eefbf5_46%,#f0f5ff_100%)] p-5 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <form className="flex w-full flex-col gap-2 sm:flex-row lg:w-[32rem]" onSubmit={(event) => { event.preventDefault(); setQuery(search.trim()); }}>
              <div className="focus-within:ring-mint/40 flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-white/70 bg-white/90 px-3 py-2 ring-2 ring-transparent">
                ...
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索问题、回答、笔记或同学" className="w-full bg-transparent text-sm font-semibold outline-none placeholder:text-slate-400" />
              </div>
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.78fr)]">
          <div className="grid gap-4">
            <CommunitySection title="推荐给我的问题" description="根据你的课程、题库和学习路径推荐，也会包含还没人回答的开放问题。" icon={<Sparkles size={18} />} accent="bg-mint/10 text-mint">
              <QuestionsPanel
                ...
                currentUserId={studentSession.id}
            ...
            <CommunitySection title="热门问题" description="大家正在讨论的问题，按回答热度和更新时间排序。" icon={<HelpCircle size={18} />} accent="bg-coral/10 text-coral">
              <QuestionsPanel
          ...
          <aside className="grid gap-4 content-start">
            <CommunitySection title="热门分享笔记" description="被同学点赞较多的复习笔记和学习方法。" icon={<NotebookPen size={18} />} accent="bg-sky-100 text-sky-700">
              <NotesPanel notes={hotNotes} currentUserId={studentSession.id} followingIds={home?.following_ids ?? []} onLikeNote={toggleNoteLike} onFollow={toggleFollow} />
            </CommunitySection>
